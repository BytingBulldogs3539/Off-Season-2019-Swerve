package frc.robot.swerve_library.math;

import java.util.Arrays;
import java.util.List;

/**
 * This is the main class for the swerveDrive library. The library handles the
 * calculations required to drive a robot using SwerveDrive wheels. These wheels
 * have two motors: A drive motor that moves the robot and a turn motor that
 * changes the wheel assembly's direction.
 *
 * MOTOR LAYOUT
 *
 * Front Wheel 2 -------------- Wheel 1 | | | | | | Left | | Right | | | | | |
 * Wheel 3 -------------- Wheel 4 Back
 *
 * The library supports two modes: Robot centric and Field centric. In Robot
 * centric mode the robot turns relative to its current position: 45 degrees to
 * the right will turn the robot 45 degrees to the right (for example, if it is
 * pointing north before the turn, it will point north-east after the turn. In
 * Field centric mode the robot turns to face the given number of degrees
 * relative to the firld's orientation: 0 means straight ahead down the field,
 * 90 means to the right, etc.
 */
public class SwerveMath
{
     ...
     * 
     * @param width
     *                   the robot width (units do not matter)
     * @param length
     *                   the robot length (units do not matter)
     */
     ...
     * 
     * @param fwd
     *                      the forward power value range -1.0(back) - 1.0(fwd)
     * @param str
     *                      the strafe power value range -1.0(left) - 1.0(right)
     * @param rcw
     *                      the rotation power value range -1.0(ccw) - 1.0(cw)
     * @param gyroValue
     *                      the value of the gyro input to be used by the
     *                      calculation. Optional. Only used when the robot is in
     *                      field-centric mode. Values are 0-360
     * @return List of wheel movement directives. The list indices correspond to the
     *         wheel numbering scheme as above, zero-based.
     */
