...
public abstract class BaseEnclosure implements SwerveEnclosure
{
    ...
    private String name;
    protected double encoderTicksPerRev;

    public BaseEnclosure(String name, double encoderTicksPerRev)
    {
        this.name = name;
    ...
    /**
     * @return the value of the angle encoder (used to calculate current wheel
     *         position) TODO: This should be converted to -1 - +1 range...
     */
    protected abstract int getEncPosition();

    /**
     * Sets the value of the angle encoder (used for aligning wheel in case of
     * drift)
     * 
     * @param encPosition
     *                        the current encoder value TODO: This should be
     *                        converted to -1 - +1 range...
     */
     ...
     * 
     * @param speed
     *                  the speed value to set: -1 - full backwards, 0 - stop, +1 -
     *                  full forward
     */
    ...
    private boolean shouldReverse(double wa, double encoderValue)
    {
        ...
        double ea = SwerveUtils.convertEncoderValue(encoderValue, encoderTicksPerRev);

        // Convert the next wheel angle, which is from -.5 to .5, to 0 to 1
        if (wa < 0)
            wa += 1;

        // Find the difference between the two (not sure if the conversion from (-0.5 to
        // 0.5) to (0 to 1) above is needed)
        // Difference between the two points. May be anything between -1 to 1, but we
        // are looking for a number between -.5 to .5
        double longDifference = Math.abs(wa - ea);

        // finds shortest distance (0 to 0.5), always positive though (which is what we
        // want)
        double difference = Math.min(longDifference, 1.0 - longDifference);

        // If the difference is greater than 1/4, then return true (aka it is easier for
        // it to turn around and go backwards than go forward)
        if (difference > 0.25)
            return true;
        else
            return false;
    ...
    private double convertAngle(double angle, double encoderValue)
    {
        ...
        double encPos = encoderValue / encoderTicksPerRev;
